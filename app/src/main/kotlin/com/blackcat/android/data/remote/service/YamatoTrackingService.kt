...
import com.blackcat.android.data.remote.parser.YamatoHtmlParser
...
class YamatoTrackingService @Inject constructor(
    private val client: OkHttpClient
) {
    companion object {
        private const val BASE_URL = "https://toi.kuronekoyamato.co.jp/cgi-bin/tneko"
    }

    suspend fun track(trackingNumber: String): TrackingResult = withContext(Dispatchers.IO) {
        try {
            val formBody = FormBody.Builder()
                .add("number01", trackingNumber)
                .build()
            ...
            val statusList = YamatoHtmlParser.parse(html)
